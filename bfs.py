...
def bfs(root):
    fringe = [] #A FIFO queue with the unexplored nodes
    visited = [] #A list with the visited nodes
    No_states = 0 #the number of past states.
...
message_bfs, time_bfs, space_bfs, depth_list_bfs, path_bfs = bfs(root) 

import plot
plot.plot_bfs(depth_list_bfs, time_bfs, space_bfs)
